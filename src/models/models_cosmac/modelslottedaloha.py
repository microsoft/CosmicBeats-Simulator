...
@desc
    This model implements a Slotted ALOHA variant for comparison with the CosMAC protocol.
    Based on the CosMAC paper: https://deepakv.web.illinois.edu/assets/papers/CosMAC_MobiCom_2024.pdf
    
    This model provides a baseline slotted ALOHA implementation for IoT satellite communication.
    It includes:
    1. Beacon-based slot synchronization
    2. Random backoff transmission scheduling
    3. Fixed transmission probability
    4. Acknowledgment-based reliability
    
    This serves as a comparison protocol to demonstrate the improvements provided
    by the CosMAC constellation-aware approach.
'''
...
class ModelSlottedAloha(IModel):
    ...
    def call_APIs(
        ...
        try:
            ...
            print(f"[ModelSlottedAloha]: An unhandled API request has been received by {self.__ownernode.nodeID}:", e)
    ...
    def Execute(self):
        """
        Main execution method for the Slotted ALOHA model.
        
        Implements a traditional slotted ALOHA protocol for comparison with CosMAC:
        1. Data generation and queuing
        2. Beacon reception for slot synchronization
        3. Random backoff slot selection
        4. Fixed probability transmission decisions
        5. Acknowledgment handling and retransmission
        
        Uses simpler probability calculations compared to CosMAC's adaptive approach.
        """
        ...
        if self.__currentState == 2:
            ...
            _beaconsReceived, self.__numDevices = self.__check_BeaconsReceived(_receivedData)
            if _beaconsReceived:
                _prob = 2.5 * self.__nSlots / (1e-4)
                if random.random() < _prob:
                    #We have received a beacon. Let's go to state 3
                    self.__currentState = 3
                #if the probability is not met, we should not send the data. Let's try again later. State remains 2
            else:
        ...
        if self.__currentState == 3:
            #We have n slots, each of which is m seconds long. 
                _backoff = np.random.randint(1, self.__nSlots)
                self.__transmitTime = self.__ownernode.timestamp.copy().add_seconds(_backoff * self.__slotLength)
    ...
    def __init__(
        ...
        '''
        Constructor for ModelSlottedAloha.
        
        Initializes the Slotted ALOHA model for baseline comparison with CosMAC.
        Uses traditional slotted ALOHA parameters and algorithms.
...
def init_ModelSlottedAloha(
    _ownernodeins: INode, 
    _loggerins: ILogger, 
    _modelArgs) -> IModel:
    '''
    Factory function to initialize a ModelSlottedAloha instance.
    
    Creates a traditional Slotted ALOHA model for comparison with CosMAC protocols.
    This model serves as a baseline to demonstrate CosMAC improvements.
    ...
    @return IModel
        Initialized instance of ModelSlottedAloha
    ...
    return ModelSlottedAloha( _ownernodeins, 
                                _loggerins,
                                2,
                                120)
